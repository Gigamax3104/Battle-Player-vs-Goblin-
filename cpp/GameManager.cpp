...
GameManager::GameManager() : m_character(new Character* [2]),m_number(new int[ENEMY + 1]) {
	m_number[PLAYER] = 1;
	m_number[ENEMY] = 1;

	m_character[PLAYER] = new Player[m_number[PLAYER]];
	m_character[ENEMY] = new Enemy[m_number[ENEMY]];
}

GameManager::GameManager(int playerNumber, int enemyNumber)
	: m_character(new Character* [playerNumber + enemyNumber]), m_number(new int[ENEMY + 1]) {
	m_number[PLAYER] = playerNumber;
	m_number[ENEMY] = enemyNumber;
...
GameManager::~GameManager() {
	if (m_character[PLAYER] != nullptr) delete[] m_character[PLAYER];
	if (m_character[ENEMY] != nullptr) delete[] m_character[ENEMY];
	if (m_character != nullptr) delete[] m_character;

	if (m_number != nullptr) delete[] m_number;
}

bool GameManager::Battle() {
	int dead(0);

	for (int i = 0; i < m_number[PLAYER] + m_number[ENEMY]; i++) {
		int index1(i / m_number[PLAYER]), index2(i % m_number[i / m_number[PLAYER]]);
		m_character[index1][index2].ParamaterShow(&m_input);
	}

	for (int i = 0; i < m_number[PLAYER] + m_number[ENEMY]; i++) {
		int index1(i / m_number[PLAYER]), index2(i % m_number[i / m_number[PLAYER]]);

		if(index1 == PLAYER) m_character[index1][index2].Choice(&m_character[(index1 + 1) % (ENEMY + 1)][index2],&m_input);
		else m_character[index1][index2].Choice(&m_character[index1 % 1][index2]);

		m_character[index1][index2].Reset();

		if (!m_character[index1][index2].IsArive()) dead++;

		if (dead == m_number[PLAYER] + m_number[ENEMY] - 1) return false;
	}
